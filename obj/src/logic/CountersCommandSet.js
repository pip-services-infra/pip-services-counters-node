...
class CountersCommandSet extends pip_services3_commons_node_1.CommandSet {
    constructor(logic) {
        super();
        this._logic = logic;
    ...
    makeReadCountersCommand() {
        return new pip_services3_commons_node_2.Command("read_counters", new pip_services3_commons_node_5.ObjectSchema(true)
            .withOptionalProperty('fitler', new pip_services3_commons_node_7.FilterParamsSchema())
    ...
    makeWriteCounterCommand() {
        return new pip_services3_commons_node_2.Command("write_counter", new pip_services3_commons_node_5.ObjectSchema(true)
            ...
            counter.time = pip_services3_commons_node_9.DateTimeConverter.toNullableDateTime(counter.time);
            this._logic.writeCounter(correlationId, counter, callback);
        });
    }
    makeWriteCountersCommand() {
        return new pip_services3_commons_node_2.Command("write_counters", new pip_services3_commons_node_5.ObjectSchema(true)
            .withRequiredProperty('counters', new pip_services3_commons_node_6.ArraySchema(new CounterV1Schema_1.CounterV1Schema())), (correlationId, args, callback) => {
            let counters = args.get("counters");
            _.each(counters, (c) => {
                c.time = pip_services3_commons_node_9.DateTimeConverter.toNullableDateTime(c.time);
            });
